...
public class ContactsController(ISender sender) : ControllerBase
{
    ...
    [HttpGet]
    [Route("Constituency")]
    [SwaggerResponse(200, "Contact", typeof(Contact))]
    [SwaggerResponse(400, "Invalid request data.")]
    public async Task<IActionResult> GetMpContactForConstituencyAsync([FromQuery] GetContactForConstituencyQuery request, CancellationToken cancellationToken)
    {
        var project = await sender.Send(request, cancellationToken);
        return Ok(project.Value);
    }
