...
public class DeleteExternalContactCommandTests
{
    ...
    private readonly Mock<IKeyContactWriteRepository> mockKeyContactWriteRepository;
    ...
    public DeleteExternalContactCommandTests()
    {
        ...
        mockKeyContactWriteRepository = new Mock<IKeyContactWriteRepository>();
        ...
        handler = new DeleteExternalContactCommandHandler(
            ...
            mockKeyContactReadRepository.Object,
            mockKeyContactWriteRepository.Object,
            mockLogger.Object);
    }
    ...
    [InlineData(null, null, "5f7f01d0-9a2f-46a6-a971-cdf153c0df88", null, "5f7f01d0-9a2f-46a6-a971-cdf153c0df88")]
    [InlineData(null, null, null, "5f7f01d0-9a2f-46a6-a971-cdf153c0df88", "5f7f01d0-9a2f-46a6-a971-cdf153c0df88")]
    public async Task Handle_ShouldDeleteExternalContact_And_KeyContact_Successfully(
       string ChairOfGovernorKeyContactId,
       string IncomingTrustCeoKeyContactId,
       string OutgoingTrustCeoKeyContactId,
       string HeadTeacherKeyContactId,
       string contactIdValue
       )
    {
        ...
        var keycontact = fixture.Build<Entities.KeyContact>()
               ...
               .With(t => t.ChairOfGovernorsId, ChairOfGovernorKeyContactId == null ? fixture.Create<ContactId>() : new ContactId(Guid.Parse(ChairOfGovernorKeyContactId)))
               .With(t => t.IncomingTrustCeoId, IncomingTrustCeoKeyContactId == null ? fixture.Create<ContactId>() : new ContactId(Guid.Parse(IncomingTrustCeoKeyContactId)))
               .With(t => t.OutgoingTrustCeoId, OutgoingTrustCeoKeyContactId == null ? fixture.Create<ContactId>() : new ContactId(Guid.Parse(OutgoingTrustCeoKeyContactId)))
               .With(t => t.HeadteacherId, HeadTeacherKeyContactId == null ? fixture.Create<ContactId>() : new ContactId(Guid.Parse(HeadTeacherKeyContactId)))
               .Create();
        ...
        var queryableKeyContacts = new List<Entities.KeyContact> { keycontact }.AsQueryable().BuildMock();
        mockKeyContactReadRepository.Setup(repo => repo.KeyContacts).Returns(queryableKeyContacts);

        mockContactWriteRepository.Setup(repo => repo.RemoveContactAsync(It.IsAny<Entities.Contact>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

        mockKeyContactWriteRepository.Setup(repo => repo.UpdateKeyContactAsync(It.IsAny<Entities.KeyContact>(), It.IsAny<CancellationToken>()))
        ...
        (
            ...
            () => Assert.True(result.IsSuccess),
            () => mockContactWriteRepository.Verify(repo => repo.RemoveContactAsync(It.IsAny<Entities.Contact>(), It.IsAny<CancellationToken>()), Times.Once),
            () => mockKeyContactWriteRepository.Verify(repo => repo.UpdateKeyContactAsync(It.IsAny<Entities.KeyContact>(), It.IsAny<CancellationToken>()), Times.Once),
            () => mockUnitOfWork.Verify(uow => uow.BeginTransactionAsync(), Times.Once),
            () => mockUnitOfWork.Verify(uow => uow.CommitAsync(), Times.Once),
            () => mockUnitOfWork.Verify(uow => uow.RollBackAsync(), Times.Never)
        );
