...
namespace Dfe.Complete.Tests.Validators
{
    public class TrnAttributeTests
    {
        ...
        [Fact]
        public async Task TrnAttribute_ShouldThrowException_WhenQueryFails()
        {
            // Arrange
            var trn = "TR12345";
            ...
            mockSender.Setup(s => s.Send(It.IsAny<GetProjectByTrnQuery>(), default))
                .ReturnsAsync(Result<GetProjectByTrnResponseDto?>.Failure(null));
            ...
            // Act & Assert
            var exception = Assert.Throws<Exception>(() => attribute.GetValidationResult(trn, validationContext));
        }
