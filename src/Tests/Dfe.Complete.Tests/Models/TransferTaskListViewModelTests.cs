...
namespace Dfe.Complete.Tests.Models
{
    public class TransferTaskListViewModelTests
    {
        ...
        {
            var model = new TransferTaskDataModel(new Domain.ValueObjects.TaskDataId(Guid.NewGuid()), handoverReview, handoverNotes, handoverMeeting, handoverNotApplicable);
            var result = TransferTaskListViewModel.HandoverWithRegionalDeliveryOfficerTaskStatus(model);
