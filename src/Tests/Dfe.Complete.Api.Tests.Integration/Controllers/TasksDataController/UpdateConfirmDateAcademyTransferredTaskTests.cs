...
namespace Dfe.Complete.Api.Tests.Integration.Controllers.TasksDataController
{
    public class UpdateConfirmDateAcademyTransferredTaskTests
    {
            ...
            IFixture fixture)
        {
            ...
            var command = new UpdateConfirmDateAcademyTransferredTaskCommand
            {
            ...
            // Act
            await tasksDataClient.UpdateConfirmDateAcademyTransferredTaskAsync(command, default);

            // Assert
            dbContext.ChangeTracker.Clear();
            var existingTaskData = await dbContext.TransferTasksData.SingleOrDefaultAsync(x => x.Id == taskData.Id);
            Assert.NotNull(existingTaskData);
            Assert.Equal(new DateOnly(2024, 1, 15), existingTaskData.ConfirmDateAcademyTransferredDateTransferred);
        }
            ...
            IFixture fixture)
        {
            ...
            var taskData = fixture.Create<TransferTasksData>();
            taskData.ConfirmDateAcademyTransferredDateTransferred = new DateOnly(2024, 1, 15); // Set initial value
            dbContext.TransferTasksData.Add(taskData);
            ...
            var existingTaskData = await dbContext.TransferTasksData.SingleOrDefaultAsync(x => x.Id == taskData.Id);
            Assert.NotNull(existingTaskData);
            Assert.Null(existingTaskData.ConfirmDateAcademyTransferredDateTransferred);
        }
    }
}
