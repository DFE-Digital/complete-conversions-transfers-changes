...
{
    public class UpdateConfirmSchoolHasCompletedAllActionsTaskTests
    {
        [Theory]
        [CustomAutoData(
            typeof(CustomWebApplicationDbContextFactoryCustomization),
            typeof(TransferTaskDataCustomization))]
        public async Task UpdateConfirmSchoolHasCompletedAllActionsTaskAsync_ShouldUpdate_ConversionTaskData(
            ...
            IFixture fixture)
        {
            ...
            // Act
            await tasksDataClient.UpdateConfirmSchoolHasCompletedAllActionsTaskAsync(command, default);

            // Assert
            dbContext.ChangeTracker.Clear();
            var existingTaskData = await dbContext.ConversionTasksData.SingleOrDefaultAsync(x => x.Id == taskData.Id);
            Assert.NotNull(existingTaskData);
            Assert.True(existingTaskData.SchoolCompletedEmailed);
        ...
        public async Task UpdateConfirmSchoolHasCompletedAllActionsTaskAsync_ShouldFail_WhenTaskDataIdNotMatched(
            CustomWebApplicationDbContextFactory<Program> factory,
            ITasksDataClient tasksDataClient,
            IFixture fixture)
        {
            ...
            // Act + Assert
            var exception = await Assert.ThrowsAsync<NotFoundException>(() => tasksDataClient.UpdateConfirmSchoolHasCompletedAllActionsTaskAsync(command, default));
