...
{
    public class UpdateCommercialAgreementTaskTests
    {
        [Theory]
        [CustomAutoData(
            ...
            IFixture fixture)
        {
            ...
            var command = new UpdateCommercialAgreementTaskCommand
            {
            ...
            Assert.True(existingTaskData.CommercialTransferAgreementConfirmSigned);
            ...
            IFixture fixture)
        {
            ...
            Assert.True(existingTaskData.CommercialTransferAgreementSigned);
            Assert.True(existingTaskData.CommercialTransferAgreementQuestionsReceived);
            Assert.True(existingTaskData.CommercialTransferAgreementQuestionsChecked);
        ...
        [CustomAutoData(
            typeof(CustomWebApplicationDbContextFactoryCustomization),
            typeof(ConversionTaskDataCustomization))]
        public async Task UpdateCommercialTransferAgreementTaskAsync_ShouldFail_WhenProjectTypeOmitted(
            CustomWebApplicationDbContextFactory<Program> factory,
            ITasksDataClient tasksDataClient,
            IFixture fixture)
        {
            ...
            // Act + Assert
            var exception = await Assert.ThrowsAsync<CompleteApiException>(() => tasksDataClient.UpdateCommercialTransferAgreementTaskAsync(command, default));
