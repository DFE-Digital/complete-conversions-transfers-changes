...
describe("Transfer Tasks - Confirm the outgoing trust CEO's details", () => {
    ...
    it.skip("Should", () => {
        cy.visit(`projects/${projectId}/tasks/confirm_outgoing_trust_ceo_contact`);
    });

    it("Should see add contact button if no ceo exists", () => {
        cy.visit(`projects/${projectWithoutCEOContactId}/tasks/confirm_outgoing_trust_ceo_contact`);
        taskPage.hasButton("Add a contact");
    });

    // awaiting 238981 to add a contact via API
    it.skip("Should NOT see the 'save and return' button for another user's project", () => {
        cy.visit(`projects/${otherUserProjectId}/tasks/confirm_outgoing_trust_ceo_contact`);
