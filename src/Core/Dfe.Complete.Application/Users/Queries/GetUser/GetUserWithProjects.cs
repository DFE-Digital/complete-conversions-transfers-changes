...
public record GetUserWithProjectsQuery(UserId UserId, ProjectState? State = ProjectState.Active) : PaginatedRequest<PaginatedResult<UserWithProjectsResultModel>>;
    ...
    : IRequestHandler<GetUserWithProjectsQuery, PaginatedResult<UserWithProjectsResultModel>>
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            foundUser.ProjectAssignedTos = foundUser.ProjectAssignedTos.Where(project => request.State == null || project.State == request.State).ToList();
            ...
            var result = new UserWithProjectsResultModel(foundUser.Id,
                ...
                foundUser.Email,
                foundUser.Team,
                foundUser.ProjectAssignedTos.Skip(request.Page*request.Count).Take(request.Count).Select(async project =>
                {
