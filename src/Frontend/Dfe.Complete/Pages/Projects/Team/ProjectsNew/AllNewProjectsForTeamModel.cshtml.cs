    ...
    public async Task OnGet()
    {
        ...
        if (UserTeamIsRdo)
        {
            ...
            var listProjectsForRegionQuery =
                new ListAllProjectsForRegionQuery((Region)userRegion!, ProjectState.Active, null, null, orderBy)
                {
                    Page = PageNumber - 1,
                    Count = PageSize
                };
        ...
        else if (userTeam == ProjectTeam.RegionalCaseWorkerServices)
        {
            var listProjectsForTeamQuery =
                new ListAllProjectsForTeamQuery(userTeam, ProjectState.Active, null, null, orderBy)
                {
                    Page = PageNumber - 1,
                    Count = PageSize
                };
