...
public class Startup
{
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        var authenticationBuilder = services
            ...
            {
                // MultiAuth handles principal resolution (incl. Cypress).
                options.DefaultScheme = "MultiAuth";
                options.DefaultAuthenticateScheme = "MultiAuth";

                // When we need to sign in or challenge, use the cookie so that it
                // goes via LoginPath (/sign-in) instead of hitting AAD immediately.
                options.DefaultSignInScheme = CookieAuthenticationDefaults.AuthenticationScheme;
                options.DefaultChallengeScheme = CookieAuthenticationDefaults.AuthenticationScheme;
            })
            .AddCypressMultiAuthentication();

        // Wire up Azure AD / Entra ID using your existing AzureAd section.
        // This is only invoked when you explicitly Challenge with this scheme (e.g. in /sign-in).
